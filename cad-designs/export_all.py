...
def main():
    ...
    part_files = [f for f in all_py if os.path.basename(f) not in excluded and "assembly" not in os.path.basename(f)]
    assembly_files = [f for f in all_py if os.path.basename(f) not in excluded and "assembly" in os.path.basename(f)]
