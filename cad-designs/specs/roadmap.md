...
- **Motorized Drive Shaft** (`part_05_motorized_shaft.py`): Model heavy-duty driven hinge shaft ($\varnothing 13.0\text{ mm}$, green) featuring an integrated press-fit 25T metal servo horn socket on the motor end for direct 1-piece drive without separate couplers (zero rotational slop), and a standardized $8.0\text{ mm}$ female hex torque socket on the output end.
...
- **Active Folding Flap** (`part_07_active_flap.py`): Model full-size active folding flap panel (red) resting on frame rails ($Z = 15.0$ to $17.4\text{ mm}$) with corner clearance cutout for the servo housing, integrated continuous $\varnothing 13.0\text{ mm}$ solid-core axle, direct servo horn spline/socket drive, standardized $8.0\text{ mm}$ female hex torque socket for follower column synchronization, $0.6\text{ mm}$ micro-grip diamond surface texture, $0.8\text{ mm}$ hole edge chamfers, $1.2\text{ mm}$ recessed perimeter shadow bevel, and gradient circular weight-reduction cutouts (~45% mass reduction, reducing blade weight to ~75g and minimizing rotational inertia on the servo).
- **Motorized Sub-Assembly** (`assembly_motorized_module.py`): Assemble Motorized Frame + Drive Shaft + Snap Cover + Active Flap + Part 10 Frame Joiners + Part 11 Hex Coupler.
